...
function sortCarInventory(inventory) {
  const sortByCarModel = (a, b) => a.car_model.localeCompare(b.car_model);
  return Array.from(inventory).sort(sortByCarModel);
}
...
function getModelYears(inventory) {
  let yearSort = inventory.map(c => c.car_year);
  return yearSort;
}
...
function getOlderCars(inventory, years) {
  let oldCar = inventory.filter(c => c.car_year <= years);
  return oldCar;
} 
...
function getGermanCars(inventory) {
  let makeOfCar = ['Audi', 'Mercedes-Benz', 'Volkswagen', 'BMW'];
  let selectCars = inventory.filter(c => makeOfCar.includes(c.car_make));
  return selectCars;
}
